...
void Font::Create(const char *faceName, int characterSet, int size, bool bold, bool italic, bool extraFontFlag) {
    ...
    wxFont* font = wxTheFontList->FindOrCreateFont( size,
                    wxDEFAULT,
...
void SurfaceImpl::Release() {
    if (bitmap) {
        ...
        wxASSERT( bitmap->GetRefData()->GetRefCount() == 1 ); 
        delete bitmap;
        bitmap = 0;
    }
    if (hdcOwned) {
        hdc->SetPen( wxNullPen );
        hdc->SetBrush( wxNullBrush );
        hdc->SetFont( wxNullFont );
        ((wxMemoryDC*)hdc)->SelectObject(wxNullBitmap);
        delete hdc;
        hdc = 0;
        hdcOwned = false;
...
void SurfaceImpl::PenColour(ColourAllocated fore) {
    wxPen* pen = wxThePenList->FindOrCreatePen(wxColourFromCA(fore), 1, wxSOLID);
    hdc->SetPen(*pen);
}

void SurfaceImpl::BrushColour(ColourAllocated back) {
    wxBrush* brush = wxTheBrushList->FindOrCreateBrush(wxColourFromCA(back), wxSOLID);
    hdc->SetBrush(*brush);
}

void SurfaceImpl::FreeBrush()
{
   hdc->SetBrush(wxNullBrush);
}

void SurfaceImpl::FreePen()
{
   hdc->SetPen(wxNullPen);
}
...
void SurfaceImpl::FillRectangle(PRectangle rc, Surface &surfacePattern) {
    if (((SurfaceImpl&)surfacePattern).bitmap)
        hdc->SetBrush( wxBrush(*((SurfaceImpl&)surfacePattern).bitmap) );
    else    
    {
       // Something is wrong so display in red
...
//
// TOM: I made this a virtual list!
//
...
public:
   ...
   int   GetMaxStrWidth() const { return m_maxStrWidth; }
...
void wxSTCListBox::SetList(const char *list,	char separator, char typesep)
{
   ...
   // Convert the string into a list of items and type values.
   wxString item;
   int type = -1;
   const char *ptr = list;
	const char *startword = list;
	int i;
   int width;
   ...
   dc.SetFont( font );
	...
	for ( ;; ptr++ ) {
      ...
      m_maxStrWidth = wxMax(m_maxStrWidth, width);
   ...
   // We're a virtual list... so we just need to
   // set the length.
   SetItemCount( m_Items.GetCount() );
   dc.SetFont( wxNullFont );
}

wxString wxSTCListBox::OnGetItemText(long item, long) const
{
   return m_Items[item];
}

int wxSTCListBox::OnGetItemImage(long item) const
{
   return m_Types[item];
}
...
public:
        ...
        wxPopupWindow(parent, wxBORDER_NONE)
    {
        ...
        lv = new wxSTCListBox(parent, id, wxDefaultPosition, wxDefaultSize,
                              wxLC_REPORT | wxLC_SINGLE_SEL | wxLC_NO_HEADER | wxBORDER_NONE | wxLC_VIRTUAL);
...
public:
        ...
        wxWindow(parent, id, wxPoint(location.x, location.y), wxSize(0,0), wxSIMPLE_BORDER )
    {
        ...
        lv = new wxSTCListBox(this, id, wxDefaultPosition, wxDefaultSize,
                              wxLC_REPORT | wxLC_SINGLE_SEL | wxLC_NO_HEADER | wxNO_BORDER | wxLC_VIRTUAL);
        lv->SetCursor(wxCursor(wxCURSOR_ARROW));
        lv->InsertColumn(0, wxEmptyString);
        lv->InsertColumn(1, wxEmptyString);
...
//WX_DECLARE_HASH_MAP( const char*, wxBitmap, wxPointerHash, wxPointerEqual, XPMBitmapHash );
...
private:
    ...
    static wxImageList  imgList;
    static wxArrayInt   imgTypeMap;

    //static XPMBitmapHash sBitmapCache;
...
wxImageList ListBoxImpl::imgList;
wxArrayInt ListBoxImpl::imgTypeMap;

//XPMBitmapHash ListBoxImpl::sBitmapCache;
...
void ListBoxImpl::Append(const wxString& text, int type) {
   ...
   // TOM: Disabled this fuction... we use SetList
   // and virtual list controls to make this thing
   // display quickly for long lists.
   wxASSERT( false );

   /*
    wxString text = stc2wx(s);
    wxListView* lb = GETLB(id);
    long count  = lb->GetItemCount();
    long itemID  = lb->InsertItem(count, wxEmptyString);
    lb->SetItem(itemID, 1, text);
    ...
    if (type != -1) {
        ...
        lb->SetItemImage(itemID, idx, idx);
    }
   */
}
...
void ListBoxImpl::SetList(const char* list, char separator, char typesep) {
   ...
   wxSTCListBox* lb = wxStaticCast(GETLB(id),wxSTCListBox);
   lb->SetList(list, separator, typesep);
   maxStrWidth = lb->GetMaxStrWidth();
...
void ListBoxImpl::RegisterImage(int type, const char *xpm_data) {
   ...
   // If we alread have this type registered... skip it!
   if ( type < imgTypeMap.Count() )
      return;

    // TOM: Added a cache for listbox images as we can have as many
    // as a dozen... multiply that by 20 open windows and you have
    // 240 held bitmap GDI objects!  This shares these handles across
    // several listbox impls.
    wxBitmap bmp;
    /*
    XPMBitmapHash::iterator itr = sBitmapCache.find(xpm_data);
    if ( itr != sBitmapCache.end() )
       bmp = itr->second;
    else
    {
    */
        ...
        bmp = wxBitmap(img);
    /*
        sBitmapCache[xpm_data] = bmp;
    }
    */

    if (imgTypeMap.IsEmpty()) {
        // assumes all images are the same size
        imgList.Create(bmp.GetWidth(), bmp.GetHeight(), true);
    }
